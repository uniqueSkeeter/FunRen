...
public class CustomerChargeServiceImpl extends AbstractBaseService implements CustomerChargeService {
	...
	public void addCustomChargeInfo(CustomerChargeBean customerChargeBean) {
		...
		// get user tradeNo from system parameter table.
		Integer tradeNo = this.customerChargeDaoImpl.getTradeNo();
		...
		// 设置客户交易号
		guestCreditEntity.setIncno(String.valueOf(tradeNo));
		...
		this.customerChargeDaoImpl.updateTradeNo();
	...
	/**
	 * 将cardChargeBean转换成FkTguestCreditEntity
	 */
	private FkTtGuestcreditEntity covertDataToGuestCreditEntity(CustomerChargeBean customerChargeBean) {
		...
		if (StringUtils.isNotBlank(ApplicationContext.getInstance().getClassChangeBean().getClassNo())) {
			guestCreditEntity.setBjid(ApplicationContext.getInstance().getClassChangeBean().getClassNo());
		}
		// 班结日期
		if (StringUtils.isNotBlank(ApplicationContext.getInstance().getClassChangeBean().getStartDate())) {
			guestCreditEntity.setAccDate(DateUtil.formatStrTOStampBySmt(ApplicationContext.getInstance().getClassChangeBean()
					.getStartDate(), DateUtil.DATE_PATTERN_8));
		}
		// 创建日期
		guestCreditEntity.setCreateDate(DateUtil.currentTimestamp());
