...
public class RegisterCustomerServiceImpl extends AbstractBaseService implements RegisterCustomerService {
	...
	public void addCardInfo(RegisterCustomerBean registerCustomerBean) {
		...
		FkTCreditEntity cardCreditEntity = this.covertDataToCardCreditEntity(registerCustomerBean);
		// get user guest number from system parameter table.
		Integer guestNo = this.registerCustomerDaoImpl.getGuestNo();
		Integer seriaNum = this.registerCustomerDaoImpl.getSeriaNum();
		...
		// 业务流水号
		cardEntity.setSeriaNum(seriaNum);
		// 班结号
		...
		this.registerCustomerDaoImpl.updateSeriaNum();
	...
	/**
	 * 将registerCustomerBean转换成fktCardCreditEntity
	 */

	private FkTCreditEntity covertDataToCardCreditEntity(RegisterCustomerBean registerCustomerBean) {
		FkTCreditEntity cardCreditEntity = new FkTCreditEntity();
		// 卡号 (不记名卡有值)(单用户卡有值)
		if (StringUtils.isNotBlank(registerCustomerBean.getCardNo())) {
			cardCreditEntity.setCardno(registerCustomerBean.getCardNo());
		}
		// 持卡人(不记名卡无值)(单用户卡有值)
		if (StringUtils.isNotBlank(registerCustomerBean.getGuestName())) {
			cardCreditEntity.setGuestname(registerCustomerBean.getGuestName());
		}
		// 交易金额(不记名卡有值)(单用户卡有值)
		if (registerCustomerBean.getCardAmount() != null) {
			cardCreditEntity.setCardinc(registerCustomerBean.getCardAmount());
		}
		// 交易前金额(不记名卡有值)(单用户卡有值)
		cardCreditEntity.setCardbeg(BigDecimal.valueOf(0.0));
		// 交易后金额(不记名卡有值)(单用户卡有值)
		if (registerCustomerBean.getCardAmount() != null) {
			cardCreditEntity.setCardbal(registerCustomerBean.getCardAmount());
		}
		// 交易类型 充值
		cardCreditEntity.setInctype("0");
		// 支付类型
		cardCreditEntity.setRectype("11");
		...
		// 操作员编号 (不记名卡有值)(单用户卡有值)
		if (StringUtils.isNotBlank(ApplicationContext.getInstance().getOperatorNo())) {
			cardCreditEntity.setUserid(Integer.valueOf(ApplicationContext.getInstance().getOperatorNo()));
		}
		// 创建时间
