...
public class CustomerDeleteDaoImpl extends AbstractBaseDAO<CardEntity> implements CustomerDeleteDAO {
	 ...
	 * 查询司机卡客户信息的SQL
	 */
	private final String driverInfoSQL = "SELECT guest.guestNo, card.guestName, card.cardNo, card.cardType, card.cardStatus, "
			+ "card.deposit, card.cardFlag, cardAcc.cardBal, cardAcc.pre, cardAcc.score "
			+ "FROM FK_T_GUEST guest, FK_T_CARD card, FK_T_CARDTZ cardAcc WHERE guest.guestNo = card.guestNo AND card.cardNo = cardAcc.cardNo ";

	/**
	 * 统计查询司机卡客户信息的SQL
	 */
	private final String countDriverInfoSQL = "SELECT count(*) FROM FK_T_GUEST guest, FK_T_CARD card, FK_T_CARDTZ cardAcc WHERE guest.guestNo = card.guestNo AND card.cardNo = cardAcc.cardNo ";
	 ...
	 * 通过客户编号改变卡状态SQL
	 */
	private final String updateCardStatusSQL = "UPDATE FK_T_CARD SET cardStatus = ? , update_Date = ? WHERE guestNo = ? ";

	/**
	 * 通过卡号改变卡状态SQL
	 */
	private final String updateCardStatusByCardNoSQL = "UPDATE FK_T_CARD SET cardStatus = ? , update_Date = ? WHERE cardNo = ? ";
	...
	public void updateCardStatus(CustomerDeleteBean customerDeleteBean) {
		Object[] args = { '4', DateUtil.currentTimestamp(), customerDeleteBean.getGuestNo() };
		// 执行SQL
	...
	public void updateCardStatusByCardNo(CustomerDeleteBean customerDeleteBean) {
		Object[] args = { '4', DateUtil.currentTimestamp(), customerDeleteBean.getCardNo() };
		// 执行SQL
	...
	public List<CustomerDeleteBean> getDriverRecordsByCriteria(CustomerDeleteBean customerDeleteBean, int startPage, int number) {
		...
		try {
			...
			// 传送查询条件
			builderForPagination.append(builder).append(" ORDER BY card.cardFlag DESC , card.cardNo ");
			// 传送查询条件
	...
	private StringBuilder validateDriverInfoInput(CustomerDeleteBean customerDeleteBean) {
		StringBuilder builder = new StringBuilder();
		if (StringUtils.isNotEmpty(ApplicationContext.getInstance().getStationNum())) {
			builder.append(" AND guest.stationNo = '").append(ApplicationContext.getInstance().getStationNum()).append("' ");
		}
		if (StringUtils.isNotEmpty(customerDeleteBean.getGuestNo())) {
			builder.append(" AND ").append("card.guestNo = '").append(customerDeleteBean.getGuestNo()).append("'");
		} else if (StringUtils.isNotEmpty(customerDeleteBean.getCardNo())) {
