...
public class CardMaintainDaoImpl extends AbstractBaseDAO<CardEntity> implements CardMaintainDAO {
	...
	/**
	 * 卡片解灰·更新FK_T_CARD表，将卡状态cardstatus置为1（启用）
	 */
	String unLockCardSql = "UPDATE FK_T_CARD SET CARDSTATUS='1',update_date=? WHERE cardNo=? ";

	/**
	 * 修改密码·更新FK_T_CARD表，修改为新密码
	 */
	String changePassSql = "UPDATE FK_T_CARD SET PASSWORD=?,update_date=?,passFlag='Y' WHERE cardNo=? ";
	...
	public List<CardMaintainBean> getUnLockCardInfo(CardMaintainBean cardMaintainBean) throws Exception {
		...
		try {
			...
			log.info("Get unlock Card data occured error, more detail please see the detail log" + e.getMessage() + "\n"
					+ ErrorLogUtil.printInfo(e));
		}
		return resultList;
	}
	...
	public List<Object[]> updateAllCardLost(CardMaintainBean cardMaintainBean) throws Exception {
		...
		try {
			String str = "SELECT cardno,cardStatus FROM FK_T_CARD WHERE guestno='" + cardMaintainBean.getGuestNo()
					+ "' AND cardstatus='1' ";
	...
	public void updateUnLockCard(CardMaintainBean cardMaintainBean) {
		Object[] args = { DateUtil.currentTimestamp(), cardMaintainBean.getCardNo() };
		this.upateBySql(this.unLockCardSql, args);
	}
