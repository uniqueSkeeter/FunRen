		...
		var cardMaintainBean = $('#card_dg').datagrid('getSelected');
		  if (!cardMaintainBean){
			  ...
			  if(cardMaintainBean.cardType=="13"){
				  $.messager.alert("错误",'不记名卡不能补卡!');
				  return ;
			  }
			  if(cardMaintainBean.cardStatus!="2"){
				  $.messager.alert("错误",'未挂失的卡不能补卡!');
				  return ;
			  }
