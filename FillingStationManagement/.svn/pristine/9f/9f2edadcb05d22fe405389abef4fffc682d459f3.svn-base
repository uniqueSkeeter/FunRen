...
public class CardMaintainServiceImpl extends AbstractBaseService implements CardMaintainService {
	...
	public void updateUnLockCard(CardMaintainBean cardMaintainBean) {
		this.cardMainDaoImpl.updateUnLockCard(cardMaintainBean);
		// 增加卡维护记录
		cardMaintainBean.setNote("卡片解灰");
		cardMaintainBean.setWorkDate(DateUtil.currentTimestamp());
		cardMaintainBean.setStationNo(String.valueOf(ApplicationContext.getInstance().getStationNum()));
		this.addFkTMaintainRecords(cardMaintainBean);
	}
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
	public List<CardMaintainBean> getUnLockCard(CardMaintainBean cardMaintainBean) throws Exception {
		if (cardMaintainBean == null) {
			return new ArrayList<CardMaintainBean>();
		}
		List<CardMaintainBean> unLockCardList = this.cardMainDaoImpl.getUnLockCardInfo(cardMaintainBean);
		return unLockCardList;
	}
