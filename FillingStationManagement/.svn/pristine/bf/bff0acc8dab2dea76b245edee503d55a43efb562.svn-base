...
public class UserLoginAction extends AbstractAction implements SessionAware, ApplicationAware, SystemConstants {
	...
	public String loginUser() {
		...
		try {
			...
			log.info("Login user failure, there is no corresponding data in DB, userName = " + this.userName);
	...
	public void setSession(Map<String, Object> session) {
		...
		this.session.put(CORP_TYPE, DataCollections.companyType);
		// for card charge
	...
	private void saveCommonInfoToSession() {
		...
		// for station name
		this.session.put(STATION_NAME, context.getStationName());
		// for station no
		this.session.put(STATION_NUM, context.getStationNum());
		// for operator number
		this.session.put(OPERATORNUM, context.getOperatorNo());
		...
		this.session.put(CLASS_NUMBER, context.getClassChangeBean().getClassNo() == null ? null : context.getClassChangeBean().getClassNo());
		// for user login name
		this.session.put(LOGIN_NAME, context.getUserName());
	}
